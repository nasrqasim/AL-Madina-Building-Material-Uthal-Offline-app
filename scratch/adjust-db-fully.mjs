...
async function main() {
  ...
  // Reset all reorder levels first to prevent accumulation, or just build up from current state
  const items = await db.collection('items').find({}).toArray();
  console.log(`Total items in database: ${items.length}`);

  // Find all items that are currently low stock
  const currentLowStock = items.filter(item => (Number(item.stockQtyCartons) || 0) <= (Number(item.reorderLevel) || 0));
  console.log(`Currently low stock: ${currentLowStock.length}`);
  ...
  // 8. Run recalculation of party balances (same as recalculate-all.mjs)
  console.log("Recalculating all party balances...");
  ...
  ]).toArray(); // Use totalAmount for salesToday to match image
