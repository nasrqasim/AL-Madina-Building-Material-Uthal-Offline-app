...
import SaleOrderDetails from "@/components/sales/SaleOrderDetails";
import ERPPageHeader from "@/components/erp/ui/ERPPageHeader";
import { Plus, Search, Filter, Eye, Edit, Trash2, ShoppingCart, CheckCircle2, Clock, Link2, Printer, FileSpreadsheet } from "lucide-react";
...
export default function SaleOrderPage() {
  ...
  return (
    <div className="space-y-6">
      <ERPPageHeader
        title="Sale Order"
        description="Manage customer sales orders and track their fulfillment."
        actions={[
          { label: "Print", onClick: printPage, icon: Printer },
          { label: "Export Excel", onClick: () => exportToExcel(orders, "SaleOrders.xlsx"), icon: FileSpreadsheet },
        ]}
      />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Order #</th>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Customer PO</th>
              ...
              {isLoading ? (
                ...
                filteredOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-maroon-800 transition-colors">{order.invoiceNo || order.orderNo}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-slate-600 dark:text-slate-300">{order.date ? order.date.split('T')[0] : "-"}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{order.partyId?.companyName || order.partyId?.name || order.customer}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-400 dark:text-slate-500">{order.reference || order.customerPo}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="text-sm font-black text-slate-900 dark:text-white">{(order.totalAmount || order.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                        order.status?.toLowerCase() === "completed" ? "bg-emerald-100 text-emerald-700" : 
                        order.status?.toLowerCase() === "approved" ? "bg-blue-100 text-blue-700" : 
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center">
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">No sale orders found.</p>
                  </td>
                </tr>
              )}
            </tbody>
