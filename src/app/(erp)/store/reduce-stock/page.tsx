...
import ReduceStockForm from "@/components/store/ReduceStockForm";
import ReduceStockDetails from "@/components/store/ReduceStockDetails";
import ERPPageHeader from "@/components/erp/ui/ERPPageHeader";
import { Plus, Search, Filter, Eye, Edit, Trash2, ClipboardList, Package, TrendingDown, Clock, AlertCircle, Printer, FileSpreadsheet } from "lucide-react";
...
interface StockAdjustment {
  ...
  docNo: string;
  date: string;
  type: "Lost" | "Damaged" | "Expired" | "Theft" | "Other";
  status: "Draft" | "Posted";
  location: string;
  totalValue: number;
  reason: string;
...
const initialAdjustments: StockAdjustment[] = [
  {
    ...
    totalValue: 4650,
    reason: "Brake fluid bottles damaged during shelf rearrangement"
  },
  {
    id: "2",
    docNo: "STK-RED-00002",
    ...
    location: "Showroom A",
    totalValue: 12000,
    reason: "Inventory mismatch found in monthly audit"
  },
  {
    id: "3",
    docNo: "STK-RED-DUMMY",
    ...
    location: "Main Warehouse",
    totalValue: 8500,
    reason: "Old batch of additive expired"
  }
];
...
export default function ReduceStockPage() {
  ...
  const fetchAdjustments = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/invoices?type=reduce_stock", { cache: "no-store" });
  ...
  if (showForm) {
    return <ReduceStockForm onClose={() => { setShowForm(false); setViewAdj(null); }} initialData={viewAdj && showForm ? viewAdj : null} />;
  }
  ...
  return (
    <div className="space-y-6">
      <ERPPageHeader
        title="Reduce Stock"
        description="Record inventory shrinkage, damages, or losses."
        actions={[
          { label: "Print", onClick: printPage, icon: Printer },
          { label: "Export Excel", onClick: () => exportToExcel(adjustments, "ReduceStock.xlsx"), icon: FileSpreadsheet },
        ]}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 bg-rose-50 text-rose-800 rounded-xl flex items-center justify-center">
            <ClipboardList size={24} />
          </div>
          <div>
            <p className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reductions</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Loss Value</p>
            <h4 className="text-2xl font-black text-rose-700 truncate">
              ...
              {isLoading ? (
                ...
                adjustments.map((adj) => (
                  <tr key={adj._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/50 transition-colors group">
                    ...
                    <td className="px-8 py-5">
                      <span className="px-2.5 py-1 bg-rose-50 text-rose-600 rounded-lg text-[10px] font-black uppercase tracking-wider border border-rose-100">
                        {adj.type}
                      </span>
                    </td>
                    <td className="px-8 py-5">
                    ...
                    <td className="px-8 py-5 text-right font-black text-rose-700">{(adj.totalAmount || adj.totalValue || 0).toLocaleString()}</td>
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => setViewAdj(adj)} className="p-1.5 text-slate-300 hover:text-rose-800 hover:bg-rose-50 rounded-lg transition-all" title="View">
                          <Eye size={16} />
                        </button>
                ...
                <tr>
                  <td colSpan={7} className="px-8 py-12 text-center text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">No stock reductions found.</td>
                </tr>
              )}
            </tbody>
          </table>
