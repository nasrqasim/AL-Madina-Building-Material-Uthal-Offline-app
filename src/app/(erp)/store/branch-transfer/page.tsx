...
import BranchTransferForm from "@/components/store/BranchTransferForm";
import BranchTransferDetails from "@/components/store/BranchTransferDetails";
import ERPPageHeader from "@/components/erp/ui/ERPPageHeader";
import { Plus, Search, Filter, Eye, Edit, Trash2, Truck, Building2, GitPullRequest, Clock, ArrowRightLeft, Printer, FileSpreadsheet } from "lucide-react";
...
const initialTransfers: BranchTransfer[] = [
  {
    id: "1",
    docNo: "BT-2026-00001",
    date: "2026-04-28",
    fromBranch: "Lahore Head Office",
    fromLocation: "Main Warehouse",
    toBranch: "Karachi Branch",
    toLocation: "Showroom A",
    status: "Dispatched",
    totalValue: 588000,
    reason: "Inter-branch stock replenishment for summer peak demand"
  },
  {
    id: "2",
    docNo: "BT-DUMMY-REF",
    date: "2026-04-30",
    fromBranch: "Karachi Branch",
    fromLocation: "Stock Yard",
    toBranch: "Lahore Head Office",
    toLocation: "Main Warehouse",
    status: "Draft",
    totalValue: 24500,
    reason: "Returning damaged inventory for central inspection"
  },
  {
    id: "3",
    docNo: "BT-TRANSIT-092",
    date: "2026-05-01",
    fromBranch: "Lahore Head Office",
    fromLocation: "Main Warehouse",
    toBranch: "Islamabad Branch",
    toLocation: "Secondary Hub",
    status: "Received",
    totalValue: 125000,
    reason: "Direct branch request #BR-922"
  }
];
...
export default function BranchTransferPage() {
  ...
  const fetchTransfers = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/invoices?type=branch_transfer", { cache: "no-store" });
  ...
  if (showForm) {
    return <BranchTransferForm onClose={() => { setShowForm(false); setViewTrf(null); }} initialData={viewTrf && showForm ? viewTrf : null} />;
  }
  ...
  return (
    <div className="space-y-6">
      <ERPPageHeader
        ...
        description="Manage stock movements between different branches."
        actions={[
          { label: "Print", onClick: printPage, icon: Printer },
          { label: "Export Excel", onClick: () => exportToExcel(transfers, "BranchTransfers.xlsx"), icon: FileSpreadsheet },
        ]}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Flows</p>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Settled</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-slate-900">
          <div className="relative flex-1 max-w-xl">
            ...
            <input
              type="text"
              placeholder="Search by branch, doc#, reason..."
              value={searchQuery}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">BT #</th>
                ...
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Routing Path (From → To)</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reason / Notes</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                        <div className="flex flex-col">
                           <span className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tighter">{trf.fromBranch || trf.branchId?.name}</span>
                           <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{trf.fromLocation || trf.fromWarehouse}</span>
                        </div>
                        <ArrowRightLeft size={14} className="text-slate-300 mx-2" />
                        <div className="flex flex-col">
                           <span className="text-sm font-black text-maroon-800 uppercase tracking-tighter">{trf.toBranch || trf.targetBranchId?.name}</span>
                           <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500">{trf.toLocation || trf.toWarehouse}</span>
                        </div>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="px-8 py-12 text-center text-slate-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">No branch transfer records found.</td>
                </tr>
              )}
            </tbody>
          </table>
