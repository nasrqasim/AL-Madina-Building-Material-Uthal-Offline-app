...
export default function ImportPurchaseInvoiceDetails({ record, onClose, onEdit }: ImportPurchaseInvoiceDetailsProps) {
  ...
  const items = [
    { 
      id: "1", 
      description: "IMPORTED LUBRICANT ADDITIVES - GRADE A", 
      qty: 1200, 
      unitPriceUSD: 12.50, 
      fcTotal: 15000.00,
      pkrTotal: 15000.00 * 278.50
    }
  ];

  const charges = [
    { name: "CUSTOMS DUTY (CD)", amount: 450000 },
    { name: "SALES TAX (ST)", amount: 820000 },
    { name: "FREIGHT & CLEARING", amount: 125000 }
  ];
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-y-8 gap-x-12">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Foreign Amount</p>
              <p className="text-sm font-black text-slate-900 dark:text-white">{record.currency} {(record.amountFC || 0).toLocaleString()}</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-bold text-slate-900 dark:text-white">1 {record.currency} = 278.50 PKR</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Landed Cost (PKR)</p>
              <p className="text-sm font-black text-maroon-800">Rs. {((record.amountFC || 0) * 278.50 + totalCharges).toLocaleString()}</p>
            </div>
          </div>
        </section>
