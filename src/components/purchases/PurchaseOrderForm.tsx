...
export default function PurchaseOrderForm({ onClose, onSave, initialData }: PurchaseOrderFormProps) {
  const [items, setItems] = useState<POItem[]>(initialData?.items || [
    { id: "1", itemId: "", description: "", qty: 1, unitPrice: 0, discPercent: 0, isTaxable: false, taxPercent: 0, total: 0 }
  ]);
  ...
  const addItem = () => setItems([...items, { id: Date.now().toString(), itemId: "", description: "", qty: 1, unitPrice: 0, discPercent: 0, isTaxable: false, taxPercent: 0, total: 0 }]);
