...
export default function ImportPurchaseInvoiceForm({ onClose, onSave, initialData }: ImportPurchaseInvoiceFormProps) {
  const [items, setItems] = useState<ItemUSD[]>(initialData?.items || [
    { id: "1", itemId: "", qty: 1, uom: "KG", weight: 0, volume: 0, unitPriceUSD: 0, foreignTotal: 0, pkrTotal: 0 }
  ]);
  ...
  const addItem = () => setItems([...items, { id: Date.now().toString(), itemId: "", qty: 1, uom: "KG", weight: 0, volume: 0, unitPriceUSD: 0, foreignTotal: 0, pkrTotal: 0 }]);
    ...
    setItems(items.map(i => {
      if (i.id === id) {
        ...
        if (field === "itemId") {
          ...
          if (selected) {
            updated.uom = selected.unit || "KG";
          }
        }
        updated.foreignTotal = (updated.qty || 0) * (updated.unitPriceUSD || 0);
  ...
  const foreignSubtotal = items.reduce((sum, i) => sum + i.foreignTotal, 0);
                ...
                {items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4 text-center">
                      <input type="number" value={item.qty} onChange={(e) => updateItem(item.id, "qty", parseFloat(e.target.value) || 0)} className="w-20 bg-transparent text-sm font-black focus:outline-none text-center border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
                    <td className="px-8 py-4 text-center">
                      <input type="number" value={item.unitPriceUSD} onChange={(e) => updateItem(item.id, "unitPriceUSD", parseFloat(e.target.value) || 0)} className="w-24 bg-transparent text-sm font-black focus:outline-none text-center border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
