...
export default function NonTaxPurchaseReturnForm({ onClose, onSave, initialData }: NonTaxPurchaseReturnFormProps) {
  const [items, setItems] = useState<NTRItem[]>(initialData?.items || [
  ...
  const subTotal = items.reduce((sum, i) => sum + (i.returnQty * i.unitPrice), 0);
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest min-w-[300px]">Item</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest w-24 text-center">Orig. Qty</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest w-24 text-center">Return Qty</th>
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4">
                      <input type="number" value={item.origQty} onChange={(e) => updateItem(item.id, "origQty", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-sm font-black focus:outline-none text-center border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
                    <td className="px-8 py-4">
                      <input type="number" value={item.returnQty} onChange={(e) => updateItem(item.id, "returnQty", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-sm font-black focus:outline-none text-center border-b border-transparent focus:border-maroon-800/30 py-2 transition-all text-maroon-800" />
                    </td>
                    <td className="px-8 py-4">
