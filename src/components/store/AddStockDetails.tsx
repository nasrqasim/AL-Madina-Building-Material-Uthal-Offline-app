...
interface AddStockDetailsProps {
  record: any;
  onClose: () => void;
  onEdit: () => void;
}

export default function AddStockDetails({ record, onClose, onEdit }: AddStockDetailsProps) {
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-screen font-sans">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-8">Inventory Inflow Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-y-8 gap-x-12">
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Adjustment Type</p>
              <p className="text-sm font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-lg inline-block">{record.type}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Adjustment Date</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white">{new Date(record.date).toLocaleDateString()}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Target Warehouse</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white font-sans">{record.location}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Valuation</p>
              <p className="text-xl font-black text-slate-900 dark:text-white tracking-tighter">Rs. {(record.totalAmount || record.totalValue || 0).toLocaleString()}</p>
            </div>
            ...
            <div className="col-span-1 md:col-span-4 space-y-2 pt-4 border-t border-slate-50">
              ...
              <p className="text-sm font-bold text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl italic">&quot;{record.remarks || record.reason}&quot;</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/30">
            <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Inflow Item Details</h3>
          </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-center">Qty Added</th>
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {items.map((item: any, index: number) => (
                  <tr key={item.id || index} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/50 transition-colors">
                    ...
                    <td className="px-8 py-6 text-sm text-emerald-600 text-center">+{item.qty}</td>
