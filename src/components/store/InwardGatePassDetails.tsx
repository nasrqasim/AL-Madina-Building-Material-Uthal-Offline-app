...
interface InwardGatePassDetailsProps {
  record: any;
  onClose: () => void;
  onEdit: () => void;
}

export default function InwardGatePassDetails({ record, onClose, onEdit }: InwardGatePassDetailsProps) {
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-screen font-sans">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            <Printer size={16} className="mr-2" /> Print Pass
          </button>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-8">Gate Entry Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-y-8 gap-x-12">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Vendor / Source</p>
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-black text-maroon-800 uppercase tracking-tighter">{record.vehicleNo}</p>
            </div>
            <div className="space-y-1">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/30">
            <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Goods Description</h3>
          </div>
        ...
        <div className="flex items-center gap-3 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
          <Clock size={14} />
          <span>Security verification logged on 2026-04-30 14:22</span>
        </div>
      </div>
    </div>
  );
