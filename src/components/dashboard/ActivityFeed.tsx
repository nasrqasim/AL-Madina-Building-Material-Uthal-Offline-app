...
import { Activity, ExternalLink, Package, ShoppingCart, Clock, Database } from "lucide-react";
...
export default function ActivityFeed() {
    ...
    if (activeTab === "Transactions") {
      const fetchTransactions = async () => {
        ...
        try {
          ...
          if (json.ok && json.data?.activities) {
  ...
  const activitiesList = [
    { title: "SQLite Engine Active", desc: "Al Madina ERP is running 100% offline with SQLite database.", time: "Live", Icon: Database, iconBg: "bg-purple-50 text-purple-600" },
  ];
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-100 dark:border-slate-800 h-full flex flex-col transition-all duration-300">
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 className="text-lg font-black text-slate-800 dark:text-white tracking-tight">Activity Feed</h2>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Recent transactions and system activities</p>
        </div>
        <Link 
          href="/reports" 
          className="flex items-center gap-1.5 text-[11px] font-bold text-slate-600 dark:text-slate-300 hover:text-maroon-800 transition-colors border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800"
        >
          View More
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
        {activeTab === "Transactions" && (transactions || []).map((item, idx) => (
          <div key={idx} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer bg-white dark:bg-slate-900 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{item.id}</p>
                  <span className={`px-2 py-0.5 rounded text-[8px] font-bold ${item.color}`}>
                    {item.status}
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{item.user}</p>
                <div className="flex items-center gap-2 text-[10px] font-medium">
                  <span className={item.typeColor}>{item.type}</span>
                  <span className="text-slate-300 dark:text-slate-700">•</span>
                  <span className="text-slate-400 dark:text-slate-500 flex items-center gap-1"><Clock size={10}/> {item.time}</span>
                </div>
              </div>
            </div>
        ...
        {activeTab === "Activities" && activitiesList.map((item, idx) => (
          <div key={idx} className="flex items-start gap-4 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors bg-white dark:bg-slate-900 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1.5">{item.desc}</p>
              <p className="text-[10px] text-slate-400 font-medium flex items-center gap-1"><Clock size={10} /> {item.time}</p>
            </div>
          </div>
        ))}
          ...
          <div className="space-y-6">
            <div className="flex justify-between px-6 pb-6 pt-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Away</p>
              </div>
              <div className="text-center">
                <p className="text-3xl font-black text-slate-300 dark:text-slate-600 mb-1">1</p>
                ...
                <div key={idx} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 ${user.status === 'online' ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-200 leading-none mb-1.5">{user.name}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{user.role}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{user.time}</p>
                  </div>
                </div>
              ))}
            </div>
