...
export default function NonTaxSaleInvoiceForm({ onClose, initialData }: NonTaxSaleInvoiceFormProps) {
  const [items, setItems] = useState<NTIItem[]>(initialData?.lines?.map((l: any, index: number) => ({
    ...
    qty: l.qty,
    priceType: l.priceType || "Retail",
    unitPrice: l.rate,
    discPercent: l.discountPercent || 0,
    total: l.netAmount
  })) || [
    { id: "1", itemId: "", description: "", qty: 1, priceType: "Retail", unitPrice: 0, discPercent: 0, total: 0 }
  ]);
  ...
  const addItem = () => setItems([...items, { id: Date.now().toString(), itemId: "", description: "", qty: 1, priceType: "Retail", unitPrice: 0, discPercent: 0, total: 0 }]);
  ...
  const updateItem = (id: string, field: keyof NTIItem, value: any) => {
    setItems(items.map(i => {
      if (i.id === id) {
        ...
        if (field === "priceType") {
          const selected = availableItems.find(ai => ai._id === i.itemId);
          if (selected) {
        ...
        updated.total = base - (base * updated.discPercent / 100);
        return updated;
      }
      return i;
    }));
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest min-w-[250px]">Item</th>
