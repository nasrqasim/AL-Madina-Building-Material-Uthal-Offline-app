...
export default function SaleOrderForm({ onClose, initialData }: SaleOrderFormProps) {
  const [items, setItems] = useState<SOItem[]>(() => {
    ...
    return [{ id: "1", itemId: "", description: "", qty: 1, unitPrice: 0, discPercent: 0, isTaxable: true, taxPercent: 0, total: 0 }];
  });
  ...
  const addItem = () => setItems(prev => [...prev, { id: Date.now().toString(), itemId: "", description: "", qty: 1, unitPrice: 0, discPercent: 0, isTaxable: true, taxPercent: 0, total: 0 }]);
  ...
  const totalTax = items.reduce((sum, i) => sum + (i.isTaxable ? (((i.qty * i.unitPrice) - ((i.qty * i.unitPrice) * i.discPercent / 100)) * i.taxPercent / 100) : 0), 0);
