...
export default function SaleReturnForm({ onClose, initialData }: SaleReturnFormProps) {
  const [items, setItems] = useState<SRItem[]>(() => {
    ...
    return [{ id: "1", itemId: "", description: "", qty: 1, unitPrice: 0, reason: "", total: 0 }];
  });
  ...
  const addItem = () => setItems(prev => [...prev, { id: Date.now().toString(), itemId: "", description: "", qty: 1, unitPrice: 0, reason: "", total: 0 }]);
      ...
      if (i.id === id) {
        ...
        if (field === "itemId") {
          ...
          if (selected) {
            ...
            updated.unitPrice = selected.retailRate || 0;
          }
        }
  ...
  const handleSave = async (status: string) => {
    ...
    const payload = {
      ...formData,
      type: "sale_return",
      invoiceNo: formData.returnNumber === "Auto-generated" ? `SR-${Date.now().toString().slice(-6)}` : formData.returnNumber,
    ...
    try {
      ...
      if (res.ok) {
        alert(`Sale Return ${isEdit ? "updated" : "saved"} successfully!`);
        onClose();
      } else {
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-screen font-sans">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-6">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">New Sale Return</h1>
          </div>
        </div>
      ...
      <div className="max-w-[1600px] mx-auto p-8 space-y-8 pb-32">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1.5">
              ...
              <input placeholder="Link to Invoice #" value={formData.invoiceRef} onChange={(e) => setFormData({...formData, invoiceRef: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-bold focus:border-maroon-800 outline-none" />
            </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reason</th>
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4">
                      <input placeholder="Reason for return" value={item.reason} onChange={(e) => updateItem(item.id, "reason", e.target.value)} className="w-full bg-transparent text-sm font-medium focus:outline-none border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
          ...
          <textarea rows={4} value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} placeholder="Notes for credit or inspection staff..." className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl text-sm font-medium focus:bg-white dark:focus:bg-slate-900 dark:bg-slate-900 focus:border-maroon-800 transition-all resize-none outline-none" />
        </section>
      </div>
    </div>
  );
