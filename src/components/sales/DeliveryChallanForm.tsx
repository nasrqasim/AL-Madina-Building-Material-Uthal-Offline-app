...
export default function DeliveryChallanForm({ onClose, initialData }: DeliveryChallanFormProps) {
  const [items, setItems] = useState<DCItem[]>(() => {
    ...
    return [{ id: "1", itemId: "", description: "", qty: 1, uom: "Units", remarks: "" }];
  });
  ...
  const addItem = () => setItems(prev => [...prev, { id: Date.now().toString(), itemId: "", description: "", qty: 1, uom: "Units", remarks: "" }]);
    ...
    setItems(items.map(i => {
      if (i.id === id) {
        ...
        if (field === "itemId") {
          ...
          if (selected) updated.description = selected.name;
        }
        return updated;
      }
      return i;
