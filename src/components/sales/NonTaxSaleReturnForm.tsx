...
export default function NonTaxSaleReturnForm({ onClose, initialData }: NonTaxSaleReturnFormProps) {
  const [items, setItems] = useState<NTSRItem[]>(initialData?.lines?.map((l: any, index: number) => ({
    ...
    { id: "1", itemId: "", description: "", origQty: 0, returnQty: 0, unitPrice: 0, total: 0 }
  ]);
  ...
  const addItem = () => setItems([...items, { id: Date.now().toString(), itemId: "", description: "", origQty: 0, returnQty: 0, unitPrice: 0, total: 0 }]);
  ...
  const updateItem = (id: string, field: keyof NTSRItem, value: any) => {
    setItems(items.map(i => {
      if (i.id === id) {
        ...
        updated.total = (updated.returnQty || 0) * (updated.unitPrice || 0);
        return updated;
      }
      return i;
    }));
  ...
  const totalReturn = items.reduce((sum, i) => sum + (i.returnQty * i.unitPrice), 0);
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 dark:bg-slate-800/50/50 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest min-w-[350px]">Description</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest w-20 text-center">Orig. Qty</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest w-20 text-center text-rose-600">Return Qty</th>
                ...
                {items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4 text-center">
                      <input type="number" value={item.origQty} onChange={(e) => updateItem(item.id, "origQty", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-sm font-bold text-center text-slate-400 dark:text-slate-500 focus:outline-none border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
                    <td className="px-8 py-4 text-center">
                      <input type="number" value={item.returnQty} onChange={(e) => updateItem(item.id, "returnQty", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-sm font-black text-center text-rose-600 focus:outline-none border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
                    <td className="px-8 py-4">
