...
export default function QuotationForm({ onClose, initialData }: QuotationFormProps) {
  const [items, setItems] = useState<QuotationItem[]>(() => {
    ...
    return [{ id: "1", itemId: "", description: "", qty: 1, priceType: "Retail", unitPrice: 0, discPercent: 0, isTaxable: false, taxPercent: 0, total: 0 }];
  });
  
  const [formData, setFormData] = useState({
    quotationNumber: initialData?.invoiceNo || "Auto-generated",
    quotationDate: initialData?.date ? new Date(initialData.date).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
    validUntil: initialData?.validUntil ? new Date(initialData.validUntil).toISOString().split("T")[0] : "",
    status: initialData?.status || "draft",
    customerId: initialData?.partyId?._id || initialData?.partyId || "",
    customerRef: initialData?.reference || "",
  ...
  const addItem = () => setItems(prev => [...prev, { id: Date.now().toString(), itemId: "", description: "", qty: 1, priceType: "Retail", unitPrice: 0, discPercent: 0, isTaxable: false, taxPercent: 0, total: 0 }]);
      ...
      if (i.id === id) {
        ...
        if (field === "itemId") {
          ...
          if (selected) {
            ...
            updated.unitPrice = updated.priceType === "Retail" ? (selected.retailRate || 0) : (selected.wholesaleRate || 0);
          }
        }
  ...
  const handleSave = async (status: string) => {
    ...
    const payload = {
      ...formData,
      type: "quotation",
      invoiceNo: formData.quotationNumber === "Auto-generated" ? `QUO-${Date.now().toString().slice(-6)}` : formData.quotationNumber,
      partyId: formData.customerId,
      reference: formData.customerRef,
    ...
    try {
      ...
      if (res.ok) {
        alert(`Quotation ${isEdit ? "updated" : "saved"} successfully!`);
        onClose();
      } else {
  ...
  const totalTax = items.reduce((sum, i) => sum + (i.isTaxable ? (((i.qty || 0) * (i.unitPrice || 0) - ((i.qty || 0) * (i.unitPrice || 0) * (i.discPercent || 0) / 100)) * (i.taxPercent || 0) / 100) : 0), 0);
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-screen font-sans">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-6">
          ...
          <div>
            <h1 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">New Customer Quotation</h1>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <button type="button" onClick={() => handleSave("posted")} className="px-4 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg flex items-center shadow-lg shadow-emerald-600/10 transition-all">
            <Send size={16} className="mr-2" /> Save & Send
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto p-8 space-y-8 pb-32">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center space-x-2 mb-8">
            <h2 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Customer & Proposal Details</h2>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-4 gap-x-8 gap-y-6">
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Select Customer *</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Valid Until *</label>
              <input type="date" value={formData.validUntil} onChange={(e) => setFormData({...formData, validUntil: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-bold focus:border-maroon-800 outline-none transition-all" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reference No</label>
              <input placeholder="Customer RFQ ref" value={formData.customerRef} onChange={(e) => setFormData({...formData, customerRef: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm font-bold focus:border-maroon-800 outline-none transition-all" />
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Employee/Sales Rep</label>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden relative">
          ...
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50/30">
            <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Estimate Line Items</h3>
              ...
              <tbody className="divide-y divide-slate-50 font-bold">
                {items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4">
                      <select value={item.priceType} onChange={(e) => updateItem(item.id, "priceType", e.target.value)} className="w-full bg-transparent text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 focus:outline-none border-b border-transparent focus:border-maroon-800/30 py-2 transition-all">
                    ...
                    <td className="px-8 py-4">
                      <input type="number" value={item.taxPercent} onChange={(e) => updateItem(item.id, "taxPercent", parseFloat(e.target.value) || 0)} className="w-full bg-transparent text-sm font-black text-center focus:outline-none border-b border-transparent focus:border-maroon-800/30 py-2 transition-all" />
                    </td>
          ...
          <div className="p-8 bg-white dark:bg-slate-900 flex flex-col items-end space-y-3">
            <div className="w-full md:w-[450px] space-y-4">
              ...
              <div className="flex justify-between text-[10px] font-black text-maroon-800 uppercase tracking-widest px-2">
                ...
                <span>-Rs. {totalDiscount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between text-[10px] font-black text-blue-600 uppercase tracking-widest px-2">
                <span>Total Tax</span>
                <span>+Rs. {totalTax.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="pt-6 border-t-2 border-slate-100 dark:border-slate-800 flex justify-between items-center px-2">
                <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.2em]">Estimate Total (PKR)</span>
                <span className="text-4xl font-black text-maroon-800 tracking-tighter">Rs. {grandTotal.toLocaleString('en-US', { minimumFractionDigits: 0 })}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
          <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Notes & Terms of Quotation</label>
          <textarea rows={6} value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} placeholder="Specify delivery time, payment terms, or price validity..." className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-3xl text-sm font-medium focus:bg-white dark:focus:bg-slate-900 dark:bg-slate-900 focus:border-maroon-800 transition-all resize-none outline-none" />
        </section>
      </div>
    </div>
  );
