...
interface NonTaxSaleInvoiceDetailsProps {
  record: any;
  onClose: () => void;
  onEdit: () => void;
}

export default function NonTaxSaleInvoiceDetails({ record, onClose, onEdit }: NonTaxSaleInvoiceDetailsProps) {
  // Mock items for the view
  const items = [
    { 
      id: "1", 
      description: "LUBRICANT OIL 4L CAN", 
      qty: 5, 
      unitPrice: 2500, 
      total: 12500 
    },
    ...
    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-screen">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
              ...
              record.status === "Posted" ? "bg-blue-100 text-blue-700" :
        ...
        <div className="flex items-center space-x-3">
          ...
          {record.status !== "Paid" && (
            <button className="px-4 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg flex items-center shadow-lg shadow-emerald-600/10 transition-all">
              <Wallet size={16} className="mr-2" /> Receive Payment
            </button>
          )}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-8">Invoice Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-y-8 gap-x-12">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Billing Date</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white">{record.date}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Reference</p>
              <p className="text-sm font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">{record.linkedRef}</p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Grand Total</p>
              <p className="text-sm font-black text-maroon-800">Rs. {record.total.toLocaleString()}</p>
            </div>

            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Balance Owed</p>
              <p className={`text-sm font-black ${record.balance > 0 ? "text-rose-600" : "text-emerald-600"}`}>
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-bold text-slate-900 dark:text-white font-sans">Main Branch</p>
            </div>
            <div className="space-y-1 md:col-span-2">
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Payment Account</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white">Cash in Hand (Standard)</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/30">
            <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Itemized Billing</h3>
          </div>
          ...
          <div className="p-8 bg-white dark:bg-slate-900 flex flex-col items-end space-y-3">
            <div className="w-full md:w-80 space-y-4">
              <div className="pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
                ...
                <span className="text-2xl font-black text-maroon-800 tracking-tighter">Rs. {record.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="flex items-center gap-3 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
          <Clock size={14} />
          <span>Last updated by Admin on 2026-04-30 14:22</span>
        </div>
      </div>
    </div>
  );
